...
(async()=>{
  const f=new NodeFile(process.argv[2]);
  const t0=Date.now();
  const info=await parseSf2(f);
  console.log(`  parsed in ${Date.now()-t0}ms:`, JSON.stringify(info));
  console.log(`  sample data: ${(SF.smplLen/1048576).toFixed(1)} MB at offset ${SF.smplOff}`);
  const banks={};
  for(const p of SF.presets) banks[p.bank]=(banks[p.bank]||0)+1;
  console.log('  presets per bank:', JSON.stringify(banks));
  // spot check a few GM programs
  for(const [bank,prog,label] of [[0,0,'Acoustic Grand'],[0,33,'Fingered Bass'],[0,48,'Strings'],[128,0,'Drum kit']]){
    const p=findPreset(bank,prog);
    const z=zonesForNote(p,60,100);
    const bytes=z.reduce((s,x)=>{const sm=SF.samples[x.sampleId];return s+(sm?(sm.end-sm.start)*2:0);},0);
    console.log(`  ${label.padEnd(16)} -> "${(p?p.name:'?').padEnd(20)}" zones@C4=${String(z.length).padStart(2)}  ${(bytes/1024).toFixed(0)}KB`);
  }
